        ...
        <form action="{{ route('course.enroll') }}" method="POST">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col mb-8">
                <div class="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    ...
                    <span class="bg-indigo-100 text-indigo-700 text-xs font-bold px-2 py-1 rounded-full border border-indigo-200">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            ...
                            <tr class="hover:bg-slate-50 transition cursor-pointer" onclick="document.getElementById('course_{{ $course->id }}').click()">
                                
                                <td class="px-6 py-4 text-center">
                                    <input type="checkbox" id="course_{{ $course->id }}" name="courses[]" value="{{ $course->id }}" 
                                        {{ in_array($course->id, $myCourses) ? 'checked' : '' }}
                                        onclick="event.stopPropagation()"
                                        class="w-5 h-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer shadow-sm">
                                </td>
                            ...
                            @empty
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-slate-400 font-medium text-base">
                ...
                <div class="p-5 border-t border-slate-100 bg-slate-50/50 flex justify-between items-center">
                    <p class="text-xs text-slate-500 font-medium max-w-md">
                        * Ders seçimlerinizi yaptıktan sonra kaydet butonuna basmayı unutmayın. Danışman onayından geçmeyen dersler kesinleşmez.
                    </p>
